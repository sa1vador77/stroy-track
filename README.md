...
Без `.env.bot` поднимутся API и база, а контейнеры бота и планировщика выйдут
с подсказкой — для работы с одним лишь API токен не нужен.
...
flowchart LR
    ...
    subgraph compose["Docker Compose"]
        ...
        bot["bot · aiogram"] --> pg
        sch["scheduler · APScheduler<br/>напоминания о несданных отчётах"] --> pg
    end
    sch -- "Bot API" --> tg
```
...
├── scheduler/    # процесс напоминаний: APScheduler-крон, рассылка через бота
...
- **Напоминания — отдельный процесс, расписание в памяти.** Изоляция сбоев:
  зависшая рассылка не трогает поллинг бота. MemoryJobStore — это at-most-once:
  дублей при рестарте не бывает, но реплика планировщика строго одна, а если
  процесс лежал в момент крона — день пропущен; `misfire_grace_time` в час
  прощает заминки живого процесса, но не простой. SIGTERM обрабатывается явно:
  `docker stop` иначе убивает процесс мимо `finally`. Время напоминания ставится в конец окна
  сдачи отчётов; календаря выходных нет — стройки работают по разным графикам.
...
- [x] Напоминания о несданных отчётах (APScheduler)
